...
const TocDropdown: React.FC<TocProps & TocDropdownProps> = (props) => {
    ...
    return (
        <>
            <div
                className={`toc-dropdown ${open ? 'toc-dropdown-open' : ''}`}
...
export const TocConnected = connect(state.mapStateToProps, state.mapDispatchToProps)(Toc);

export const TocContainerLeft: React.FC = (props) => (
    <div className="toc-container">{props.children}</div>
);
export const TocPage: React.FC = (props) => <div className="toc-page">{props.children}</div>;
